...
## 10.4 UI/UX 设计系统（2026-05-27 重构）

#### 设计 Token

新增统一的设计 Token，用于保持视觉一致性：
...
从 5 个 Tab 重构为 3 个 Tab + 独立流程：

**主 Tab（3 个）：**
- **今日**：Hero 卡片 + AI Coach + 快速添加 + 下一步专注
- **任务**：任务列表 + 搜索筛选 + 回收站
- **我的**：个人信息 + 复盘统计 + 学习资源库 + 设置

**独立流程（不占 Tab）：**
- **专注计时器**：从今日/任务点击后进入全屏计时器
- **资源详情**：从"我的"进入的二级页面
...
.backgroundColor(CARD_GLASS) // 半透明白色
.backdropBlur(CARD_GLASS_BLUR) // 背景模糊
.borderRadius(RADIUS.lg) // 大圆角
.shadow(SHADOW_CARD) // 柔和阴影
```
