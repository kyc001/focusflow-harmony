...
tasks & id、title、note、project\_id、priority、due\_at、status、pomodoro\_count、tags\_json、subtasks\_json & 保存任务主体、优先级、截止时间、完成状态、番茄数量、标签和子任务等信息。 \\
pomodoros & id、task\_id、start\_at、duration、completed、interrupt\_reason、updated\_at、client\_request\_id & 保存一次专注过程的开始时间、持续时间、完成状态、中断原因和同步幂等标识。 \\
study\_resources & id、title、source\_type、project\_id、linked\_task\_id、content、summary、key\_points\_json、status、review\_due\_at & 保存学习资料、AI 提炼结果、关联任务和复习时间。 \\
\bottomrule
\end{tabularx}
\end{table}
...
projects & id、user\_id、name、color、icon、updated\_at、is\_deleted & 按用户保存项目数据，并通过更新时间支持增量同步。 \\
tasks & id、user\_id、title、project\_id、priority、status、client\_request\_id、tags\_json、subtasks\_json & 保存服务端任务数据，客户端请求标识用于降低重复提交风险。 \\
pomodoros & id、user\_id、task\_id、start\_at、duration、completed、interrupt\_reason、client\_request\_id & 保存服务端番茄记录，用于统计和跨设备同步。 \\
\bottomrule
\end{tabularx}
\end{table}
...
并发与响应性方面，界面层通过 async/await 等异步机制调用数据库、网络和文件选择能力，使耗时操作不直接阻塞 UI 绘制。复盘页涉及周统计、项目分布、连续学习天数和植物解锁等多项聚合计算，系统在刷新数据后使用 TaskPool 对统计快照进行后台预计算，并保留同步计算回退路径；页面读取的是预计算后的结果，从而减少大量番茄记录下重复扫描数组对主线程的影响。云同步采用“先拉取、再上推”的串行流程，避免同一时间发起多个同步请求造成状态交叉；番茄记录和任务状态更新先在本地完成，再进入同步流程，使用户操作反馈优先于远程结果。对于 POST 非幂等问题，任务和番茄同步数据包含 clientRequestId，可用于识别重复提交并降低重复写入风险。客户端拉取服务端番茄记录时会按 clientRequestId 查询已有记录，并依据 updatedAt 保留较新的版本；后端上推接口同样会在发现相同 clientRequestId 时执行更新时间比较后更新已有记录，避免多次同步产生重复番茄，从而保证复盘统计不会被重复数据放大。
