...
国内外关于学习管理和效率工具的研究与产品实践大体可以分为四类。第一类是待办和项目管理工具，例如 Microsoft To Do、滴答清单和 Trello 等，它们强调任务清单、截止日期和提醒机制，适合把目标拆解为行动项，但通常不直接保存学习资料，也难以把资料阅读过程与任务执行记录关联起来。第二类是笔记和知识库工具，例如 Notion、OneNote 和语雀等，它们适合沉淀课程笔记、网页摘录和实验记录，但任务执行、番茄计时和复盘统计往往需要依赖外部工具完成。第三类是专注和番茄钟工具，例如 Forest、番茄 ToDo 等，它们通过时间块、视觉激励和中断控制帮助用户保持注意力，相关研究也表明番茄工作法对学习节奏管理具有积极作用\cite{santiago2022pomodoro,cirillo2018pomodoro}，但这类工具通常只记录时间，不理解时间背后的课程资料和任务上下文。第四类是 AI 学习助手和智能辅导系统，它们能够根据输入内容生成摘要、解释和建议\cite{kasneci2023chatgpt,wang2024aitutor}，但如果缺少本地数据组织和错误兜底，容易受网络、接口稳定性和隐私边界影响。

从研究角度看，移动学习已经被证明能够扩展学习发生的时间和空间\cite{crompton2018use,sung2016effects}，但学习工具的碎片化仍然明显。学生常常需要在笔记、待办、计时器、网盘和聊天记录之间来回切换，导致资料、行动和结果分散。近年来 local-first 软件思想提出应优先保证用户在本地拥有数据和核心操作能力，云端更多承担同步与协作增强角色\cite{kleppmann2019localfirst,turnbull2023localfirst}。这一思想与大学生学习场景高度契合：课程资料和任务应在离线状态下可访问，网络和 AI 服务则应作为增强能力而不是基本流程的前置条件。
